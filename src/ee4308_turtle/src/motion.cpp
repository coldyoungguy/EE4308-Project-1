...
int main(int argc, char **argv)
{
    ...
    else
    {
        ...
        double imu_lin_vel, odom_lin_vel, odom_ang_vel;
        double wheel_l_prev = wheel_l, wheel_r_prev = wheel_r;
        double d_wheel_l, d_wheel_r;
        double turn_rad, ang_rbt_prev = ang_rbt;
        double true_pos_x, true_pos_y, true_ang, siny_cosp, cosy_cosp;
        auto &q = msg_odom.pose.pose.orientation;
        double error_x, error_y, error_ang, error_dist;
        ...
        while (ros::ok() && nh.param("run", true))
        {
            ...
            /////////////////// GROUND TRUTH ///////////////////////
            true_pos_x = msg_odom.pose.pose.position.x;
            true_pos_y = msg_odom.pose.pose.position.y;
            auto &q = msg_odom.pose.pose.orientation;
            siny_cosp = 2 * (q.w * q.z + q.x * q.y);
            cosy_cosp = 1 - 2 * (q.y * q.y + q.z * q.z);
            true_ang = atan2(siny_cosp, cosy_cosp);
            error_x = pos_rbt.x - true_pos_x;
            error_y = pos_rbt.y - true_pos_y;
            error_dist = sqrt(pow(error_x, 2) + pow(error_y, 2));
            error_ang = ang_rbt - true_ang;

            ////////////////// MOTION FILTER HERE //////////////////

            // Odom Measurements
            d_wheel_l = wheel_l - wheel_l_prev;
            d_wheel_r = wheel_r - wheel_r_prev;
            odom_lin_vel = wheel_radius / 2 / dt * (d_wheel_r + d_wheel_l);
            odom_ang_vel = wheel_radius / axle_track / dt * (d_wheel_r - d_wheel_l);
            ...
            ang_rbt = limit_angle(ang_rbt);

            if (abs(ang_vel) > straight_thresh)
            {
                pos_rbt.x = pos_rbt.x + turn_rad * (sin(ang_rbt) - sin(ang_rbt_prev));
                pos_rbt.y = pos_rbt.y + turn_rad * (cos(ang_rbt_prev) - cos(ang_rbt));
            }
            else
            {
            ...
            if (verbose)
            {
                ...
                ROS_WARN("ERROR: Error_Pos(%7.3f) Error_Ang(%6.7f)",
                         error_dist, error_ang);
                // ROS_ERROR("Ang_rbt: (%6.3f)", ang_rbt);
                // ROS_WARN("Ang err: (%6.7f)", error_ang);
            }
